...
void Brres::Tex0::openBytestream(giga::Bytestream& bytestream) {
    ...
    this->resolveOffsets(bytestream);

    nameOffset = bytestream.readScalar<std::int32_t>();
...
void Brres::Tex0::resolveOffsets(giga::Bytestream& bytestream) {
...
void Brres::Tex0::parseImageData(giga::Bytestream& bytestream) {
    ...
    std::uint32_t bpp, Bpp;

    giga::Bytestream blocks;

    std::uint32_t j, k, m, n;
    for(std::uint32_t i = 0; i < offsetCount; i++) {
        if(i == 0) { // image data section
            std::uint32_t imageStartOff = std::abs(outerBrresOffset) + offsets[0];
            bytestream.seek(imageStartOff);

            switch(_format) {
                case Format::I4: {
                    std::uint8_t tmpPixelBuf;
                    std::uint8_t tmpPixel[0x4];
                    ...
                    blockCount = (_width * _height * 4) / 0x40;
                    bpp = 0x20;
                    Bpp = 0x4;
