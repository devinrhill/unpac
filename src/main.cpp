...
usage:
        ...
        std::cerr << "Usage: unpac <x|c|d|C|L|m|P> <filename>" << std::endl;
        return 1;
    }
    ...
    if(argc >= 3) {
        ...
        if(program == "x") {
            ...
            */
            ...
            std::ifstream inListFile("/home/devinrhill/Code/proj/workstation/unpac/list.json");
            if(std::filesystem::exists("/home/devinrhill/Code/proj/workstation/unpac/list.json")) {
            ...
            outListFile.open("/home/devinrhill/Code/proj/workstation/unpac/list.json");
            ...
            input = argv[2];
            output = std::filesystem::path(argv[2]).parent_path() / std::filesystem::path(std::string(argv[2]) + ".arc");
            ...
            Vector3 scale = {positionFactor * 0.27f, positionFactor * 0.5f, positionFactor * 0.5f};
            ...
            while(!WindowShouldClose()) {
                ...
                BeginDrawing();
                    ...
                    BeginMode3D(camera.getCamera());
                        ...
                        for(unpac::g03::Item& item: itemSet) {
                            //DrawCubeV({item.x * positionFactor, item.y * positionFactor, item.z * positionFactor}, scale, GOLD);
                            DrawSphereEx({item.x * positionFactor, item.y * positionFactor, item.z * positionFactor}, scale.x, 6, 10, GOLD);
                        }
                    EndMode3D();
                EndDrawing();
            }
